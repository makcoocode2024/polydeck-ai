...
impl CdpClient {
    ...
    pub async fn probe(&self, port: u16) -> Result<Vec<CdpTarget>, CdpError> {
        let base =
            Url::parse(&format!("http://127.0.0.1:{port}")).map_err(|_| CdpError::Malformed)?;
        if !is_loopback(base.host_str()) {
            return Err(CdpError::NonLoopback);
        }
        let _version: CdpVersion = self
            .http
            .get(
                base.join("/json/version")
                    .map_err(|_| CdpError::Malformed)?,
            )
            .send()
            .await
            .map_err(|_| CdpError::Unavailable)?
            .json()
            .await
            .map_err(|_| CdpError::Malformed)?;
        self.http
            .get(base.join("/json/list").map_err(|_| CdpError::Malformed)?)
            .send()
            .await
            .map_err(|_| CdpError::Unavailable)?
            .json::<Vec<CdpTarget>>()
    ...
    pub fn select_codex_target(targets: &[CdpTarget]) -> Result<CdpTarget, CdpError> {
        targets
            ...
            .filter(|t| {
                t.target_type == "page"
                    && t.websocket_url.as_deref().is_some_and(valid_websocket_url)
            })
            .find(|t| {
    ...
    pub async fn install_fixed_script(
        &self,
        target: &CdpTarget,
        script: &str,
    ) -> Result<(), CdpError> {
        ...
        socket
            ...
            .await
            .map_err(|e| CdpError::Transport(e.to_string()))?;
        while let Some(message) = tokio::time::timeout(self.timeout, socket.next())
            .await
            .map_err(|_| CdpError::Timeout)?
        {
            ...
            if let Message::Text(text) = message {
                let response: Value =
                    serde_json::from_str(&text).map_err(|_| CdpError::Malformed)?;
...
mod tests {
    ...
    fn accepts_loopback_ws() {
        assert!(valid_websocket_url("ws://127.0.0.1:9222/devtools/page/1"));
    }
    ...
    fn selects_codex_page() {
        let target = CdpClient::select_codex_target(&[
            CdpTarget {
                id: "1".into(),
                title: "DevTools".into(),
                url: "devtools://devtools".into(),
                target_type: "page".into(),
                websocket_url: Some("ws://127.0.0.1:1/1".into()),
            },
            CdpTarget {
                id: "2".into(),
                title: "Codex".into(),
                url: "app://openai-codex/".into(),
                target_type: "page".into(),
                websocket_url: Some("ws://127.0.0.1:1/2".into()),
            },
        ])
        .unwrap();
