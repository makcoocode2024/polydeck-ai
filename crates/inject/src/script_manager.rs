...
impl ScriptManager {
    ...
    pub fn set_script_source(&mut self, source: impl Into<String>) {
        self.script_source = source.into();
    }
    ...
    pub fn enable(&self) -> ScriptManagerResult<ScriptStatus> {
        self.require_owned_script()?;
        self.update_manifest(true)?;
        self.status()
    }
    pub fn disable(&self) -> ScriptManagerResult<ScriptStatus> {
        self.require_owned_script()?;
        self.update_manifest(false)?;
        self.status()
    }
    pub fn uninstall(&self) -> ScriptManagerResult<ScriptStatus> {
        if self.paths.script.exists() {
            self.require_owned_script()?;
            fs::remove_file(&self.paths.script)?;
        }
        if self.paths.ownership.exists() {
            fs::remove_file(&self.paths.ownership)?;
        }
    ...
    pub fn status(&self) -> ScriptManagerResult<ScriptStatus> {
        ...
        let actual_hash = fs::read_to_string(&self.paths.script)
            .ok()
            .map(|s| hash(&s));
        ...
        let enabled = self
            .read_manifest()
            .ok()
            .and_then(|v| v.get("scripts")?.get(SCRIPT_KEY)?.as_bool())
            .unwrap_or(false);
        Ok(ScriptStatus {
            available,
            installed,
    ...
    fn write_owned_script(&self) -> ScriptManagerResult<()> {
        storage::atomic_write(&self.paths.script, self.script_source.as_bytes())?;
        Ok(())
    }
    fn write_ownership(&self) -> ScriptManagerResult<()> {
        storage::atomic_write_json(
            ...
            &OwnershipManifest {
                ...
                script_hash: hash(&self.script_source),
            },
        )?;
        Ok(())
    }
    fn require_owned_script(&self) -> ScriptManagerResult<()> {
        ...
        if ownership.version != OWNERSHIP_VERSION
            || ownership.script_key != SCRIPT_KEY
            || ownership.script_hash != hash(&self.script_source)
            || fs::read_to_string(&self.paths.script)
                .map(|s| hash(&s))
                .ok()
                .as_deref()
                != Some(ownership.script_hash.as_str())
        {
    ...
    fn update_manifest(&self, enabled: bool) -> ScriptManagerResult<()> {
        ...
        let scripts = manifest
            .as_object_mut()
            .ok_or(ScriptManagerError::MalformedManifest)?
            .entry("scripts")
            .or_insert_with(|| Value::Object(Map::new()))
            .as_object_mut()
            .ok_or(ScriptManagerError::MalformedManifest)?;
        scripts.insert(SCRIPT_KEY.to_string(), Value::Bool(enabled));
        storage::atomic_write_json(&self.paths.manifest, &manifest)?;
        Ok(())
    }
...
pub fn validate_selector_overrides(value: &Value) -> ScriptManagerResult<BTreeMap<String, String>> {
    let object = value.as_object().ok_or_else(|| {
        ScriptManagerError::InvalidSelectorOverride("root must be an object".into())
    })?;
    let allowed = [
        "pluginMarketUnlock",
        "sessionActions",
    ...
    for (feature, selector) in object {
        if !allowed.contains(&feature.as_str()) {
            return Err(ScriptManagerError::InvalidSelectorOverride(format!(
                "unknown feature '{feature}'"
            )));
        }
        let selector = selector.as_str().ok_or_else(|| {
            ScriptManagerError::InvalidSelectorOverride(format!("'{feature}' must be a string"))
        })?;
        if selector.is_empty() || selector.len() > 512 || selector.contains('\0') {
            return Err(ScriptManagerError::InvalidSelectorOverride(format!(
                "'{feature}' has an unsafe selector"
            )));
        }
...
fn validate_manifest(value: &Value) -> ScriptManagerResult<()> {
    ...
    for key in ["enabled", "scripts", "market"] {
        if let Some(v) = object.get(key) {
            ...
            if !valid {
                return Err(ScriptManagerError::MalformedManifest);
            }
        }
    }
    Ok(())
...
fn default_manifest() -> Value {
    serde_json::json!({"enabled": true, "scripts": {}, "market": {}})
}
...
mod tests {
    ...
    fn selector_override_accepts_known_features() {
        let result =
            validate_selector_overrides(&serde_json::json!({"stepwise":"[data-input]"})).unwrap();
