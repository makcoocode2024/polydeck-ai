...
impl CredentialResolver for KeyringCredentialResolver {
    fn credential_for(&self, key_id: &str) -> Result<Option<String>, StepwiseError> {
        polydeck_core::credentials::get_credential(key_id)
            .map_err(|_| StepwiseError::MissingCredential)
    }
}
...
impl StepwiseCache {
    pub fn get(&mut self, key: &str) -> Option<Vec<String>> {
        ...
        if value.created.elapsed() >= CACHE_TTL {
...
pub struct StepwiseService<R: CredentialResolver> {
    resolver: R,
...
impl<R: CredentialResolver> StepwiseService<R> {
    ...
    pub async fn suggestions(
        ...
        if context.trim().is_empty() {
            return Err(StepwiseError::InvalidConfig("context is empty".into()));
        }
        ...
        let credential = self
            .resolver
            .credential_for(&provider.id)?
            .ok_or(StepwiseError::MissingCredential)?;
        let url = format!(
            "{}/chat/completions",
        ...
        let response =
            tokio::time::timeout(Duration::from_secs(config.timeout_seconds), request.send())
                .await
                .map_err(|_| StepwiseError::Timeout)?
                .map_err(|_| StepwiseError::Request)?;
        let body: Value = response
            .json()
            .await
            .map_err(|_| StepwiseError::InvalidResponse)?;
...
fn validated_settings(
...
) -> Result<ValidatedSettings, StepwiseError> {
    if !(1..=MAX_SUGGESTIONS).contains(&settings.suggestion_count) {
        return Err(StepwiseError::InvalidConfig(
    ...
    if !(1..=120).contains(&settings.timeout_secs) {
        return Err(StepwiseError::InvalidConfig(
    ...
    Ok(ValidatedSettings {
        ...
        timeout_seconds: settings.timeout_secs as u64,
    })
}
...
fn parse_suggestions(body: &Value, count: u8) -> Result<Vec<String>, StepwiseError> {
    let content = body
        ...
        .ok_or(StepwiseError::InvalidResponse)?;
    let parsed: Value =
    ...
    if suggestions.is_empty() {
        return Err(StepwiseError::InvalidResponse);
    }
    Ok(suggestions)
}
