...
use crate::{
    BridgeHandler, BridgeRequest, BridgeResponse, BridgeServer, CdpClient, ScriptManager,
    ScriptPaths, ScriptStatus,
};
...
impl InjectionManager {
    pub fn new(script_source: impl Into<String>) -> Self {
        ...
        let scripts = ScriptManager::new(
            ScriptPaths::platform_default()
                .unwrap_or_else(|| ScriptPaths::from_root(".ai-deck-codex")),
            script_source.clone(),
        );
        let native = scripts
            .status()
            .unwrap_or_else(|_| unavailable_script_status());
        Self {
            scripts,
            script_source,
            bridge: None,
            status: InjectStatus {
                stage: InjectionStage::Stopped,
                channel: InjectionChannel::None,
                native,
                message: None,
            },
        }
    ...
    pub async fn start(
    ...
    ) -> Result<InjectStatus, String> {
        if !settings.enabled {
            return Err("Codex enhancement is disabled.".into());
        }
        if self.bridge.is_none() {
            self.bridge = Some(
                BridgeServer::start(handler)
                    .await
        ...
        self.notify(json!({"kind":"configure","config":renderer_config(settings)}))
            .await;
        Ok(self.status.clone())
    }

    pub async fn attach_verified_cdp(
        &mut self,
        settings: &InjectionSettings,
        port: u16,
        handler: BridgeHandler,
        ...
        let bootstrap = self
            .bridge
            .as_ref()
            .expect("bridge started above")
            .bootstrap_source();
        let rendered_script =
            self.script_source
                .replacen("(() => {", &format!("{bootstrap}\n(() => {{"), 1);
        client
            .install_fixed_script(&target, &rendered_script)
            .await
    ...
    pub async fn bridge_status(&self) -> Option<crate::BridgeStatus> {
        match &self.bridge {
            Some(bridge) => Some(bridge.status().await),
            None => None,
        }
    }
...
mod tests {
    ...
    fn unavailable_requests_have_safe_error() {
        let response = unsupported_bridge_request(BridgeRequest {
            id: "request-1".into(),
            command: crate::BridgeCommand::Ack,
            payload: Value::Null,
        });
        assert!(!response.ok);
