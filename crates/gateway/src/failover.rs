...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ProviderHealth { Healthy, Degraded, Failed, CircuitOpen, Unknown }
...
impl HealthStatus {
    fn unknown(provider_id: String) -> Self {
        Self {
            provider_id, state: ProviderHealth::Unknown, circuit: CircuitState::Closed,
...
impl Default for FailoverOptions {
    fn default() -> Self {
        Self {
            threshold: 3, recovery_threshold: 2,
            check_interval: Duration::from_secs(30),
            cooldown: Duration::from_secs(60), auto_failback: false,
        }
    }
}
...
impl FailoverSlot {
    ...
    pub fn from_manager(manager: Arc<FailoverManager>) -> Self {
        Self { manager: Arc::new(RwLock::new(Some(manager))) }
    }
    pub async fn replace(&self, manager: Option<Arc<FailoverManager>>) -> Option<Arc<FailoverManager>> {
...
impl FailoverManager {
    pub fn new(
        profile_id: String, primary: ProviderConfig,
        backups: Vec<ProviderConfig>, options: FailoverOptions,
        ...
        for provider in std::iter::once(&primary).chain(backups.iter()) {
            ...
            clients.insert(provider.id.clone(), UpstreamClient::new(
                provider.base_url.clone(), provider.api_key.clone(),
                Duration::from_secs(120), 0,
            )?);
            health.insert(provider.id.clone(), HealthStatus::unknown(provider.id.clone()));
        }
        let initial = FailoverStatus {
            profile_id: profile_id.clone(), running: false,
            primary_provider_id: primary.id.clone(),
            current_provider_id: primary.id.clone(),
            on_backup: false, all_providers_failed: false,
        ...
        Ok(Arc::new(Self {
            ...
            state: Arc::new(RwLock::new(State {
                current_provider_id: primary.id.clone(),
                health, opened_at: HashMap::new(), history: Vec::new(),
                running: false, all_providers_failed: false,
            })),
            primary, backups, options, clients, status_tx,
    ...
    pub async fn report_failure(&self, provider_id: &str, error: impl Into<String>) -> Option<String> {
        ...
        let mut should_switch = false;
        {
            ...
            if let Some(health) = state.health.get_mut(provider_id) {
                ...
                if health.consecutive_failures >= self.options.threshold {
                    ...
                    state.opened_at.insert(provider_id.to_string(), Instant::now());
                    should_switch = state.current_provider_id == provider_id;
                }
            }
            self.publish_locked(&state);
        }
        if should_switch { self.switch_to_next(provider_id, &error).await } else { None }
    }

    async fn switch_to_next(&self, from: &str, reason: &str) -> Option<String> {
        let candidates: Vec<String> = self.chain()
            .map(|p| p.id.clone()).filter(|id| id != from).collect();
        for candidate in candidates {
            if !self.can_attempt(&candidate).await { continue; }
    ...
    async fn can_attempt(&self, provider_id: &str) -> bool {
        ...
        match circuit {
            Some(CircuitState::Open) => {
                let cooled = state.opened_at.get(provider_id)
                    .map(|at| at.elapsed() >= self.options.cooldown).unwrap_or(true);
                if cooled {
                    if let Some(health) = state.health.get_mut(provider_id) {
    ...
    async fn probe(&self, provider_id: &str) -> Result<u64, String> {
        let provider = self.chain().find(|p| p.id == provider_id)
        ...
        let models = client.get(models_url).bearer_auth(&provider.api_key).send().await;
        ...
        match result {
            Ok(()) => { self.report_success(provider_id, latency).await; Ok(latency) }
            Err(error) => { self.report_probe_failure(provider_id, &error, latency).await; Err(error) }
        }
    }
    ...
    async fn report_probe_failure(&self, provider_id: &str, error: &str, latency: u64) {
        ...
        if let Some(health) = state.health.get_mut(provider_id) {
            ...
            if health.consecutive_failures >= self.options.threshold || health.circuit == CircuitState::HalfOpen {
                health.circuit = CircuitState::Open;
                health.state = ProviderHealth::CircuitOpen;
                state.opened_at.insert(provider_id.to_string(), Instant::now());
            }
    ...
    pub async fn start(self: &Arc<Self>) {
        ...
        *self.shutdown.lock().await = Some(tx);
        { let mut state = self.state.write().await; state.running = true; self.publish_locked(&state); }
    ...
    async fn monitor_once(&self) {
        ...
        if self.can_attempt(&current).await {
            if let Err(error) = self.probe(&current).await {
                let failures = self.state.read().await.health.get(&current)
                    .map(|h| h.consecutive_failures).unwrap_or(0);
                if failures >= self.options.threshold {
                    self.switch_to_next(&current, &error).await;
                }
            }
        }
        if self.options.auto_failback && current != self.primary.id
    ...
    pub async fn stop(&self) {
        if let Some(tx) = self.shutdown.lock().await.take() { let _ = tx.send(()); }
    }

    pub fn subscribe(&self) -> watch::Receiver<FailoverStatus> { self.status_tx.subscribe() }
    ...
    pub async fn history(&self, limit: u32) -> Vec<FailoverEvent> {
        self.state.read().await.history.iter().rev().take(limit as usize).cloned().collect()
    }

    fn snapshot(&self, state: &State) -> FailoverStatus {
        let providers: Vec<_> = self.chain()
            .filter_map(|p| state.health.get(&p.id).cloned()).collect();
        FailoverStatus {
            profile_id: self.profile_id.clone(), running: state.running,
            ...
            all_providers_failed: state.all_providers_failed, providers,
        }
    }
    ...
    pub async fn send(&self, endpoint: Endpoint, body: Value) -> Result<(String, reqwest::Response), UpstreamError> {
        let (provider_id, client) = self.current_client().await
            .map_err(|error| UpstreamError { message: error, never_sent: true })?;
        let start = Instant::now();
        match client.send(endpoint, body).await {
            Ok(response) if response.status().is_server_error()
                || response.status() == reqwest::StatusCode::TOO_MANY_REQUESTS => {
                let status = response.status();
                self.report_failure(&provider_id, format!("upstream returned {}", status)).await;
                Ok((provider_id, response))
            }
            Ok(response) => {
                self.report_success(&provider_id, start.elapsed().as_millis() as u64).await;
                Ok((provider_id, response))
            }
            Err(error) => {
                self.report_failure(&provider_id, error.message.clone()).await;
                Err(error)
            }
        }
    }
...
mod tests {
    ...
    fn provider(id: &str) -> ProviderConfig {
        ProviderConfig { id: id.into(), name: id.into(), base_url: "http://127.0.0.1:9".into(), api_key: "k".into(), default_model: "m".into() }
    }

    fn manager(threshold: u32) -> Arc<FailoverManager> {
        FailoverManager::new("profile".into(), provider("primary"), vec![provider("backup")],
            FailoverOptions { threshold, ..Default::default() }).unwrap()
    }
    ...
    fn rejects_zero_threshold() {
        assert!(FailoverManager::new("p".into(), provider("a"), vec![],
            FailoverOptions { threshold: 0, ..Default::default() }).is_err());
    }

    #[test]
    fn rejects_duplicate_provider_ids() {
        assert!(FailoverManager::new("p".into(), provider("a"), vec![provider("a")],
