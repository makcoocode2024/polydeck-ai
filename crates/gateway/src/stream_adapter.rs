...
impl StreamAdapter {
    pub fn new(model: String) -> Self {
        Self {
            model, response_id: String::new(),
            accumulated_content: String::new(), accumulated_thinking: String::new(),
            tool_calls: Vec::new(), text_started: false,
        }
    }

    pub fn start(&mut self) -> Vec<String> {
        if self.response_id.is_empty() { self.response_id = "temp-id".to_string(); }
        let response = json!({
            "id": self.response_id, "object": "response", "created_at": 0,
            "status": "in_progress", "model": self.model,
            "output": [], "parallel_tool_calls": true
        });
        vec![self.event("response.created", json!({"type": "response.created", "response": response}))]
    }

    pub fn push_chat_chunk(&mut self, chunk: &Value) -> Vec<String> {
        let mut events = Vec::new();
        if let Some(id) = chunk.get("id").and_then(Value::as_str) {
            if self.response_id.is_empty() { self.response_id = id.to_string(); }
        }
        ...
        if let Some(content) = delta.get("content").and_then(Value::as_str) {
            ...
            events.push(self.event("response.output_text.delta", json!({
        ...
        if let Some(tool_calls) = delta.get("tool_calls").and_then(Value::as_array) {
            for tool_call in tool_calls {
                ...
                while self.tool_calls.len() <= index {
                    self.tool_calls.push(ToolCallState { id: String::new(), name: String::new(), arguments: String::new() });
                }
                let state = &mut self.tool_calls[index];
                if let Some(id) = tool_call.get("id").and_then(Value::as_str) { state.id = id.to_string(); }
                if let Some(function) = tool_call.get("function") {
                    if let Some(name) = function.get("name").and_then(Value::as_str) { state.name = name.to_string(); }
                    if let Some(args) = function.get("arguments").and_then(Value::as_str) { state.arguments.push_str(args); }
                }
            }
        }
        events
    ...
    pub fn finish(&self) -> Vec<String> {
        ...
        vec![self.event("response.completed", json!({"type": "response.completed", "response": response}))]
    }

    fn item_id(&self) -> String {
        if self.response_id.is_empty() { "msg_0".to_string() }
        else { format!("{}_msg", self.response_id) }
    }
...
mod tests {
    ...
    fn emits_responses_completion_event() {
        ...
        adapter.push_chat_chunk(&json!({"id":"chatcmpl-123","choices":[{"delta":{"content":"hi"}}]}));
    ...
    fn emits_output_text_delta_for_content() {
        let mut adapter = StreamAdapter::new("test-model".to_string());
        let events = adapter.push_chat_chunk(&json!({"id":"chatcmpl-123","choices":[{"delta":{"content":"hi"}}]}));
